...
const { loadPartialConfig, loadPartialConfigAsync } = require('@babel/core')
const {
  resolveSync: resolveTsConfigSync,
  resolve: resolveTsConfigAsync
} = require('tsconfig')
...
const getVueJestConfigAsync = async function getVueJestConfig(jestConfig) {
  return (
    (jestConfig && jestConfig.globals && jestConfig.globals['vue-jest']) || {}
  )
}
...
const getBabelOptionsAsync = async function loadBabelOptionsAsync(
  filename,
  options = {}
) {
  const opts = Object.assign(options, {
  ...
  const config = await loadPartialConfigAsync(opts)
...
const getTsJestConfig = function getTsJestConfig(config) {
  const tsConfigPath = path.resolve(
    process.cwd(),
    getVueJestConfig(config).tsconfig || getVueJestConfig(config).tsConfig || ''
  )
...
const getTsJestConfigAsync = async function getTsJestConfig(config) {
  const tsConfigPath = path.resolve(
    process.cwd(),
    getVueJestConfig(config).tsconfig || getVueJestConfig(config).tsConfig || ''
  )

  const isUsingTs = await resolveTsConfigAsync(tsConfigPath)
  if (!isUsingTs) {
    return null
  }
...
const loadSrcAsync = async (src, filePath) => {
  var dir = path.dirname(filePath)
  var srcPath = path.resolve(dir, src)
  try {
